...
class SebastianPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shape_list_filename, patch_radius, points_per_patch, patch_features,
        ...
        self.output_eval_data = output_eval_data
    ...
    def __getitem__(self, index):
        ...
        if self.output_eval_data:
            return (patch_pts.float(),) + patch_feats + (trans.float(),) + (self.shape_names[index],) + (ctr_i,)
        else:
